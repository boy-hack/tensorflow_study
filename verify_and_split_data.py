#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# @Time    : 2019/11/20 11:32 AM
# @Author  : w8ay
# @File    : verify_and_split_data.py
...
import os
import shutil
import conf.trainer_conf as conf
...
def split(origin_dir, train_dir, test_dir, bad_imgs):
    ...
    print("开始分离原始图片集为：测试集（{}%）和训练集（{}%）".format(conf.test_percentage * 100, (1 - conf.test_percentage) * 100))
    ...
    # 测试集
    test_count = int(total_count * conf.test_percentage)
    test_set = set()
...
def main():
    # 图片路径
    origin_dir = conf.origin_image_dir
    train_dir = conf.train_image_dir
    test_dir = conf.test_image_dir
    # 图片尺寸
    real_width = conf.image_width
    real_height = conf.image_height
    # 图片后缀
    image_suffix = conf.image_suffix
