...
from conf.trainer_conf import GEN_CHAR_SET, test_image_dir, train_image_dir, IMAGE_CHANNEL, RESIZE, train_batch_size
from lib.constants import RunMode
from lib.pretreatment import preprocessing

PATH_MAP = {
    RunMode.Trains: train_image_dir,
    RunMode.Test: test_image_dir
}
...
class DataIterator:
    ...
    def _encoder(code):
        ...
        try:
            code = os.path.basename(code).split("_")[0]
    ...
    def read_sample_from_files(self, data_set=None):
        if data_set:
            ...
            try:
                self._label_list = [
                    self._encoder(i) for i in data_set
                ]
            ...
            for root, sub_folder, file_list in os.walk(self.data_dir):
                for file_path in file_list:
                    ...
                    code = os.path.basename(image_name).split("_")[0]
    ...
    def _image(path_or_bytes, is_random=False):
        ...
        im = preprocessing(im, -1, -1, -1).astype(np.float32)
    ...
    def generate_batch_by_files(self, image_index=None):
        ...
        if image_index:
            ...
            for i, index in enumerate(image_index):
                try:
                    is_training = len(image_index) == train_batch_size
